...
void ROSPixelCloudFusionApp::CloudCallback(const sensor_msgs::PointCloud2::ConstPtr &in_cloud_msg)
{
  ...
  for (size_t i = 0; i < in_cloud->points.size(); i++)
  {
    ...
    cv::Mat disp_img = current_frame_;
    cv::circle(disp_img, cv::Point(u, v), 1, cv::Scalar(255, 255, 255), 1, -1);
    cv::imshow("Image", disp_img);
    cv::waitKey(1);
  }
  pub_pixels.publish(msg);
...
void ROSPixelCloudFusionApp::InitializeROSIo(ros::NodeHandle &in_private_handle)
{
  ...
  ros::NodeHandle nh;
  pub_pixels = nh.advertise<std_msgs::Int64MultiArray>("pixels", 1000);
